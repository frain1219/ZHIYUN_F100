...
/*!
 * @brief   UART 0x7F Detection Judgment
 *
 * @param   None
 *
 * @retval  None
...
void USART_Wait7F(void)
{
    ...
    while(GPIOA->IDATA & BIT10);
    SysTick->VAL = SysTick_LOAD_RELOAD_Msk;

    while(!(GPIOA->IDATA & BIT10));
    while(GPIOA->IDATA & BIT10);

    temp = (SysTick_VAL_CURRENT_Msk - (SysTick->VAL) + 1);

    USART1->BR = temp / 8;

    USART_Read(dat, 1);

    while (!(USART1->STS_B.TXBEFLG));
    USART1->DATA = 0x0079;
}
