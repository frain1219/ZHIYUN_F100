 ...
 * @retval   None
 */
void DAC_Config(DAC_CHANNEL_T channel, DAC_Config_T* dacConfig)
{
    if(channel == DAC_CHANNEL_1)
    {
        DAC->CTRL_B.TRGSELCH1   = dacConfig->trigger;
        DAC->CTRL_B.WAVENCH1    = dacConfig->waveGeneration;
        DAC->CTRL_B.MAMPSELCH1  = dacConfig->maskAmplitudeSelect;
        DAC->CTRL_B.BUFFDCH1    = dacConfig->outputBuff;
        
    }
    
    if(channel == DAC_CHANNEL_2)
    {
        DAC->CTRL_B.TRGSELCH2   = dacConfig->trigger;
        DAC->CTRL_B.WAVENCH2    = dacConfig->waveGeneration;
        DAC->CTRL_B.MAMPSELCH2  = dacConfig->maskAmplitudeSelect;
        DAC->CTRL_B.BUFFDCH2    = dacConfig->outputBuff;
    }
}
...
{
    dacConfig->trigger              = DAC_TRIGGER_T6_TRGO;
...
void DAC_EnableWaveGeneration(DAC_CHANNEL_T channel, DAC_WAVE_GENERATION_T wave)
{
    if(channel == DAC_CHANNEL_1)
    {
        DAC->CTRL_B.WAVENCH1 = wave;
    }
    else
    {
        DAC->CTRL_B.WAVENCH2 = wave;
    }
}
 ...
 *                      @arg DAC_INT_CH2_DMAUDR: DMA Channel2 underrun interrupt mask,
 *
