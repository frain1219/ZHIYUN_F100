...
void USB_DevUserApplication(void)
{
    ...
    switch (userAppState)
    {
        ...
        case USER_APP_RUN:
            ...
            {
                /* Loopback */
                if(gCDCVCP[CDC1].state == USBD_CDC_VCP_REV_UPDATE)
                {
                    gCDCVCP[CDC1].state = USBD_CDC_VCP_IDLE;
                    
                    USBD_FS_CDC_ItfSend((uint8_t *)cdcRxBuffer[CDC1], (uint16_t)gCDCVCP[CDC1].rxUpdateLen, CDC1);
                    
                    USBD_FS_CDC_ItfSend((uint8_t *)"\r\n", strlen("\r\n"), CDC1);
                }
                /* Loopback */
                if(gCDCVCP[CDC2].state == USBD_CDC_VCP_REV_UPDATE)
                {
                    gCDCVCP[CDC2].state = USBD_CDC_VCP_IDLE;
                    
                    USBD_FS_CDC_ItfSend((uint8_t *)cdcRxBuffer[CDC2], (uint16_t)gCDCVCP[CDC2].rxUpdateLen, CDC2);
