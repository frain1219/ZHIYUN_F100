...
USBH_HANDLE_T usbHSHostHandler;
USBH_HANDLE_T usbFSHostHandler;
...
void USBH_HardwareInit(USBH_INFO_T* usbInfo)
{
    ...
    if (usbInfo->hostSpeed == USBH_SPEED_FS)
    {
        ...
        /* Link structure */
        usbFSHostHandler.usbGlobal    = USB_OTG_FS;
        usbFSHostHandler.usbHost      = USB_OTG_FS_H;
        usbFSHostHandler.usbFifo      = USB_OTG_FS_FIFO;
        usbFSHostHandler.usbPower     = USB_OTG_FS_PWR;

        /* Link data */
        usbFSHostHandler.dataPoint    = usbInfo;
        usbInfo->dataPoint          = &usbFSHostHandler;

        usbFSHostHandler.usbCfg.mode              = USB_OTG_MODE_HOST;
        usbFSHostHandler.usbCfg.phyType           = USB_OTG_PHY_EMB;
        usbFSHostHandler.usbCfg.dmaStatus         = DISABLE;
        usbFSHostHandler.usbCfg.sofStatus         = DISABLE;
        usbFSHostHandler.usbCfg.speed             = USB_OTG_SPEED_FSLS;
        usbFSHostHandler.usbCfg.hostChannelNum    = 8;
        ...
        USB_OTG_DisableAllGlobalInterrupt(usbFSHostHandler.usbGlobal);

        /* Init USB Core */
        USBH_Config(&usbFSHostHandler);
    ...
    else if (usbInfo->hostSpeed == USBH_SPEED_HS)
    {
        ...
        /* Link structure */
        usbHSHostHandler.usbGlobal    = USB_OTG_HS;
        usbHSHostHandler.usbHost      = USB_OTG_HS_H;
        usbHSHostHandler.usbFifo      = USB_OTG_HS_FIFO;
        usbHSHostHandler.usbPower     = USB_OTG_HS_PWR;

        /* Link data */
        usbHSHostHandler.dataPoint    = usbInfo;
        usbInfo->dataPoint          = &usbHSHostHandler;

        usbHSHostHandler.usbCfg.mode              = USB_OTG_MODE_HOST;
        usbHSHostHandler.usbCfg.phyType           = USB_OTG_PHY_EMB;
        usbHSHostHandler.usbCfg.dmaStatus         = DISABLE;
        usbHSHostHandler.usbCfg.sofStatus         = DISABLE;
        usbHSHostHandler.usbCfg.speed             = USB_OTG_SPEED_HSFSLS;
        usbHSHostHandler.usbCfg.hostChannelNum    = 12;
        ...
        /* Disable USB OTG all global interrupt */
        USB_OTG_DisableAllGlobalInterrupt(usbHSHostHandler.usbGlobal);

        /* Init USB Core */
        USBH_Config(&usbHSHostHandler);
