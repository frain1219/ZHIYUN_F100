...
USBD_STA_T USBD_HS_WINUSB_ItfInit(void)
{
    ...
    USBD_WINUSB_ConfigRxBuffer(&gUsbDeviceHS, winusbRxBuffer);
    USBD_WINUSB_ConfigTxBuffer(&gUsbDeviceHS, winusbTxBuffer, 0);
...
USBD_STA_T USBD_HS_WINUSB_ItfSend(uint8_t *buffer, uint16_t length)
{
    ...
    USBD_WINUSB_INFO_T *usbDevCDC = (USBD_WINUSB_INFO_T*)gUsbDeviceHS.devClass[gUsbDeviceHS.classID]->classData;
    ...
    USBD_WINUSB_ConfigTxBuffer(&gUsbDeviceHS, buffer, length);
    
    usbStatus = USBD_WINUSB_TxPacket(&gUsbDeviceHS);
...
USBD_STA_T USBD_HS_WINUSB_ItfReceive(uint8_t *buffer, uint32_t *length)
{
    ...
    USBD_WINUSB_ConfigRxBuffer(&gUsbDeviceHS, &buffer[0]);
    
    USBD_WINUSB_RxPacket(&gUsbDeviceHS);
