...
int main(void)
{
    ...
    printf("USB Device WINUSB HS2 Application\r\n");
...
void USB_DevUserApplication(void)
{
    ...
    {
        case USER_APP_INIT:
            interval = USBD_WINUSB_ReadInterval(&gUsbDeviceHS);
        ...
        case USER_APP_RUN:
            ...
            if(gUsbVCP.state == USBD_WINUSB_VCP_REV_UPDATE)
            {
                ...
                USBD_HS_WINUSB_ItfSend((uint8_t *)winusbRxBuffer, (uint16_t)gUsbVCP.rxUpdateLen);
                
                USBD_HS_WINUSB_ItfSend((uint8_t *)"\r\n", strlen("\r\n"));
            }
            else
            {
                i++;
                sprintf(str,"Hello %d\r\n",i);
                USBD_HS_WINUSB_ItfSend((uint8_t *)str, strlen(str));
            }
