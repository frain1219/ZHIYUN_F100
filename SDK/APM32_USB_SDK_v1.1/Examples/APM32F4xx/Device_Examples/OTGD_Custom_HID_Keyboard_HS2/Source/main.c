...
/** @addtogroup Examples
  * @brief OTGD Custom HID Keyboard HS2 examples
  @{
  */
...
/** @defgroup OTGD_Custom_HID_Keyboard_HS2_Macros Macros
  @{
*/

#define DEBUG_USART    USART1

/**@} end of group OTGD_Custom_HID_Keyboard_HS2_Macros*/

/** @defgroup OTGD_Custom_HID_Keyboard_HS2_Enumerates Enumerates
  @{
  */
...
/**@} end of group OTGD_Custom_HID_Keyboard_HS2_Enumerates*/
...
void USB_DevUserApplication(void)
{
    ...
    {
        case USER_APP_INIT:
            interval = USBD_CUSTOM_HID_ReadInterval(&gUsbDeviceHS);
            ...
            if (!APM_EVAL_PBGetState(BUTTON_KEY1))
            {
                ...
                if (!APM_EVAL_PBGetState(BUTTON_KEY1))
                {
                    if(i > 29)
                    {
                        ...
                        USBD_CUSTOM_HID_TxReport(&gUsbDeviceHS, (uint8_t*)report, 8);
                        APM_DelayMs(20);
                        report[2] = 0;
                        USBD_CUSTOM_HID_TxReport(&gUsbDeviceHS, (uint8_t*)report, 8);
                        APM_DelayMs(20);
