...
/** @addtogroup OTGD_CDC_DualCore
  @{
  */

/** @defgroup OTGD_CDC_DualCore_Macros Macros
  @{
  */
...
/**@} end of group OTGD_CDC_DualCore_Macros */

/** @defgroup OTGD_CDC_DualCore_Variables Variables
  @{
  */

USBD_HANDLE_T usbHSDeviceHandler;

USBD_HANDLE_T usbFSDeviceHandler;

/**@} end of group OTGD_CDC_DualCore_Variables*/

/** @defgroup OTGD_CDC_DualCore_Functions Functions
  @{
  */
...
void USBD_HardwareInit(USBD_INFO_T* usbInfo)
{
    ...
    if (usbInfo->devSpeed == USBD_SPEED_FS)
    {
        ...
        /* Link structure */
        usbFSDeviceHandler.usbGlobal    = USB_OTG_FS;
        usbFSDeviceHandler.usbDevice    = USB_OTG_FS_D;
        usbFSDeviceHandler.usbFifo      = USB_OTG_FS_FIFO;
        usbFSDeviceHandler.usbPower     = USB_OTG_FS_PWR;

        /* Link data */
        usbFSDeviceHandler.dataPoint    = usbInfo;
        usbInfo->dataPoint            = &usbFSDeviceHandler;

        usbFSDeviceHandler.usbCfg.mode                = USB_OTG_MODE_DEVICE;
        usbFSDeviceHandler.usbCfg.phyType             = USB_OTG_PHY_EMB;
        usbFSDeviceHandler.usbCfg.dmaStatus           = DISABLE;
        usbFSDeviceHandler.usbCfg.sofStatus           = DISABLE;
        usbFSDeviceHandler.usbCfg.speed               = USB_OTG_SPEED_FSLS;
        usbFSDeviceHandler.usbCfg.speedChannel        = USBD_SPEED_CH_FS;
        usbFSDeviceHandler.usbCfg.devEndpointNum      = 4;
        usbFSDeviceHandler.usbCfg.lowPowerStatus      = DISABLE;
        usbFSDeviceHandler.usbCfg.powerManageStatus   = DISABLE;
        usbFSDeviceHandler.usbCfg.vbusSense           = DISABLE;
        usbFSDeviceHandler.usbCfg.ep1Status           = DISABLE;
        ...
        NVIC_EnableIRQRequest(OTG_FS_IRQn, 2, 0);

        /* Disable USB OTG all global interrupt */
        USB_OTG_DisableAllGlobalInterrupt(usbFSDeviceHandler.usbGlobal);

        /* Init USB Core */
        USBD_Config(&usbFSDeviceHandler);

        /* Configure FIFO */
        /* Configure RX FIFO */
        USB_OTG_ConfigRxFifoSize(usbFSDeviceHandler.usbGlobal, USBD_FS_RX_FIFO_SIZE);

        /* Configure TX FIFO */
        USBD_OTG_ConfigDeviceTxFifo(&usbFSDeviceHandler, 0, USBD_FS_TX_FIFO_0_SIZE);
        USBD_OTG_ConfigDeviceTxFifo(&usbFSDeviceHandler, 3, USBD_FS_TX_FIFO_1_SIZE);
    ...
    else if (usbInfo->devSpeed == USBD_SPEED_HS)
    {
        ...
        /* Link structure */
        usbHSDeviceHandler.usbGlobal    = USB_OTG_HS;
        usbHSDeviceHandler.usbDevice    = USB_OTG_HS_D;
        usbHSDeviceHandler.usbFifo      = USB_OTG_HS_FIFO;
        usbHSDeviceHandler.usbPower     = USB_OTG_HS_PWR;

        /* Link data */
        usbHSDeviceHandler.dataPoint    = usbInfo;
        usbInfo->dataPoint            = &usbHSDeviceHandler;

        usbHSDeviceHandler.usbCfg.mode                = USB_OTG_MODE_DEVICE;
        usbHSDeviceHandler.usbCfg.phyType             = USB_OTG_PHY_EMB;
        usbHSDeviceHandler.usbCfg.dmaStatus           = DISABLE;
        usbHSDeviceHandler.usbCfg.sofStatus           = DISABLE;
        usbHSDeviceHandler.usbCfg.speed               = USB_OTG_SPEED_HSFSLS;
        usbHSDeviceHandler.usbCfg.speedChannel        = USBD_SPEED_CH_HS;
        usbHSDeviceHandler.usbCfg.devEndpointNum      = 6;
        usbHSDeviceHandler.usbCfg.lowPowerStatus      = DISABLE;
        usbHSDeviceHandler.usbCfg.powerManageStatus   = DISABLE;
        usbHSDeviceHandler.usbCfg.vbusSense           = DISABLE;
        usbHSDeviceHandler.usbCfg.ep1Status           = DISABLE;
        ...
        /* Disable USB OTG all global interrupt */
        USB_OTG_DisableAllGlobalInterrupt(usbHSDeviceHandler.usbGlobal);

        /* Init USB Core */
        USBD_Config(&usbHSDeviceHandler);

        /* Configure FIFO */
        /* Configure RX FIFO */
        USB_OTG_ConfigRxFifoSize(usbHSDeviceHandler.usbGlobal, USBD_HS_RX_FIFO_SIZE);

        /* Configure TX FIFO */
        USBD_OTG_ConfigDeviceTxFifo(&usbHSDeviceHandler, 0, USBD_HS_TX_FIFO_0_SIZE);
        USBD_OTG_ConfigDeviceTxFifo(&usbHSDeviceHandler, 1, USBD_HS_TX_FIFO_1_SIZE);
...
/**@} end of group OTGD_CDC_DualCore_Functions */
/**@} end of group OTGD_CDC_DualCore */
