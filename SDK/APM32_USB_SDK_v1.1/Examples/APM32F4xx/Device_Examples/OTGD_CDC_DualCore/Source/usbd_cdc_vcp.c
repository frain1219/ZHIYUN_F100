...
uint8_t hscdcTxBuffer[USBD_CDC_TX_BUF_LEN];
uint8_t hscdcRxBuffer[USBD_CDC_RX_BUF_LEN];

uint8_t fscdcTxBuffer[USBD_CDC_TX_BUF_LEN];
uint8_t fscdcRxBuffer[USBD_CDC_RX_BUF_LEN];
...
USBD_CDC_VCP_INFO_T gUsbHSVCP;

USBD_CDC_VCP_INFO_T gUsbFSVCP;
...
USBD_STA_T USBD_HS_CDC_ItfInit(void)
{
    ...
    USBD_CDC_ConfigRxBuffer(&gUsbDeviceHS, hscdcRxBuffer);
    USBD_CDC_ConfigTxBuffer(&gUsbDeviceHS, hscdcTxBuffer, 0);
...
USBD_STA_T USBD_HS_CDC_ItfReceive(uint8_t *buffer, uint32_t *length)
{
    ...
    gUsbHSVCP.state = USBD_CDC_VCP_REV_UPDATE;
    gUsbHSVCP.rxUpdateLen = *length;
...
USBD_STA_T USBD_FS_CDC_ItfInit(void)
{
    ...
    USBD_CDC_ConfigRxBuffer(&gUsbDeviceFS, fscdcRxBuffer);
    USBD_CDC_ConfigTxBuffer(&gUsbDeviceFS, fscdcTxBuffer, 0);
...
USBD_STA_T USBD_FS_CDC_ItfReceive(uint8_t *buffer, uint32_t *length)
{
    ...
    gUsbFSVCP.state = USBD_CDC_VCP_REV_UPDATE;
    gUsbFSVCP.rxUpdateLen = *length;
