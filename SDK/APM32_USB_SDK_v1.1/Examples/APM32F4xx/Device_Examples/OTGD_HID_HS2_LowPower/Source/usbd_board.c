...
/** @defgroup OTGD_HID_HS2_LowPower_Macros Macros
  @{
  */
...
/**@} end of group OTGD_HID_HS2_LowPower_Macros */
...
void USBD_HardwareInit(USBD_INFO_T* usbInfo)
{
    ...
    else if (usbInfo->devSpeed == USBD_SPEED_HS)
    {
        ...
        /*!< When use HS2 of internal PHY for low power wake-up
        or computer power-on restart, lowPowerStatus needs to be set */
...
void USBD_SuspendCallback(USBD_HANDLE_T* usbdh)
{
    ...
    {
        /* Mount the usb bus to an external interrupt */
        EINT_Config_T     EINT_configStruct;
        /* Enable the SYSCFG Clock */
        RCM_EnableAPB2PeriphClock(RCM_APB2_PERIPH_SYSCFG);
        SYSCFG_ConfigEINTLine(SYSCFG_PORT_GPIOB, SYSCFG_PIN_15);
        ...
        NVIC_EnableIRQRequest(EINT15_10_IRQn, 0x00, 0x00);
