...
class GDBMCPServer:
    ...
    def __init__(self, gdb_path: str = "gdb", connection: Optional[str] = None):
        ...
        self.gdb_manager = GDBManager(gdb_path, connection)
...
async def main(gdb_path: str = "gdb", connection: Optional[str] = None):
    """Main entry point."""
    server = GDBMCPServer(gdb_path, connection)
    await server.run()
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Remote connection address (e.g., localhost:3333)"
    )
    return parser.parse_args()


def run():
    """Synchronous entry point with argument parsing."""
    args = parse_args()
    asyncio.run(main(args.gdbpath, args.connection))
