...
class GDBSession:
    ...
    def __init__(self, session_id: str, timeout: int = 300, gdb_path: str = "gdb", connection: Optional[str] = None):
        self.id = session_id
...
class GDBManager:
    ...
    def __init__(self, gdb_path: str = "gdb", connection: Optional[str] = None):
        self.gdb_path = gdb_path
        self.connection = connection
    ...
    async def create_session(self, timeout: int = 300) -> str:
        ...
        async with self._lock:
            session = GDBSession(session_id, timeout, self.gdb_path, self.connection)
            await session.start()
            self.sessions[session_id] = session
            
            if self.connection:
                # Renode / Cortex-M stubs: extended-remote matches typical
                # `arm-none-eabi-gdb` + `target extended-remote :3333` workflow.
                await session.send_command(f"target extended-remote {self.connection}")
